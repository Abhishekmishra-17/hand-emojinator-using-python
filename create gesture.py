...
def store_images(g_id):
    total_pics = 1200
    #open webcam by opencv (cv2) module
    cap = cv2.VideoCapture(2,cv2.CAP_DSHOW) #also use "cap=cv2.VideoCapture(0)" if webcam support it arguments are 1,-1,0,2.. 
    x, y, w, h = 300, 50, 350, 350

    createFolder("gesture/"+str(g_id))
    pic_no = 0
    ...
    while True:
        ...
        if ret:              #check return value,i.e. if camera is open then value is return ,otherwise condition is false
            ...
            if len(contours)>0:
                    ...
                    if keypress == ord('c'):    # press key 'c' to start capturing of gesture 
                    ...
                    if pic_no == total_pics:
                        break
                    if cv2.waitKey(27) & 0xFF==ord('q'):  # break all the windows related webcam ...
                        break 
